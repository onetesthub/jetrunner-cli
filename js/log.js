...
const Log = {
	Label: (name) => console.log(`\n${sym.info} ${name}\n`),
	Success: ({ name, statusCode, time }) => console.log(`${sym.success} ${green(` ${name} `)} ${green(statusCode)} ${green(` ${time} `)}`),
	Fail: ({ name, statusCode, time }) => console.log(`${sym.error} ${red(` ${name} `)} ${red(statusCode)} ${red(` ${time} `)}`),
};
module.exports = { Log };
